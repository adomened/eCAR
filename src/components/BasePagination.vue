<template>
  <ul class="pagination" :class="[size && `pagination-${size}`, align && `justify-content-${align}`]">
    ...
    <li class="page-item" :class="{active: value === item}"
      ...
      <a class="page-link" :class="[{ 'border-0': value === item }, { 'bg-dark': value === item && color === 'default' },
      { 'bg-default': value === item && color === 'blue' }]" @click="changePage(item)">{{item}}</a>
    </li>
...
export default {
  ...
  props: {
    ...
    color: {
      type: String,
      default: 'default',
      description: 'color of the select page'
    },
    value: {
      type: Number,
      default: 1,
